...
function WooCommerceSection() {
  const store = useStore();
  const saved = getSavedWooCommerceConfig?.() ?? null;
  ...
  const handleSaveConfig = () => {
    if (!setWooCommerceConfig) {
      toast.error("WooCommerce configuration module not available.");
      return false;
    }
  ...
  const handleTestConnection = async () => {
    ...
    if (!testConnection) {
      toast.error("WooCommerce sync module not available.");
      return;
    }
    setTesting(true);
  ...
  const handleFullSync = async () => {
    if (!handleSaveConfig()) return;
    if (!fullSyncFromWooCommerce) {
      toast.error("WooCommerce sync module not available.");
      return;
    }
    setSyncing(true);
    try {
      const result = await fullSyncFromWooCommerce((stage: string, current: number, total: number) => {
